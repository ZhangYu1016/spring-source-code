...
public class EvaluationTests extends AbstractExpressionTests {
	...
	public void testAdvancedNumerics() throws Exception {
		int twentyFour = (int) parser.parseExpression("2.0 * 3e0 * 4").getValue(Integer.class);
		assertEquals(24, twentyFour);
		double one = (double) parser.parseExpression("8.0 / 5e0 % 2").getValue(Double.class);
		assertEquals(1.6d, one, 0);
		int o = (int) parser.parseExpression("8.0 / 5e0 % 2").getValue(Integer.class);
		assertEquals(1, o);
		int sixteen = (int) parser.parseExpression("-2 ^ 4").getValue(Integer.class);
		assertEquals(16, sixteen);
		int minusFortyFive = (int) parser.parseExpression("1+2-3*8^2/2/2").getValue(Integer.class);
	...
	public void testComparison() throws Exception {
		...
		boolean trueValue = (boolean) parser.parseExpression("T(java.util.Date) == Birthdate.Class").getValue(context,
			Boolean.class);
	...
	public void increment02postfix() {
		...
		double return_ddd = (double) e.getValue(ctx,Double.TYPE);
		...
		float return_fff = (float) e.getValue(ctx,Float.TYPE);
		...
		long return_lll = (long) e.getValue(ctx,Long.TYPE);
		...
		int return_iii = (int) e.getValue(ctx,Integer.TYPE);
		assertEquals(42,return_iii);
		assertEquals(43,helper.iii);
		return_iii = (int) e.getValue(ctx,Integer.TYPE);
		...
		short return_sss = (short) e.getValue(ctx,Short.TYPE);
	...
	public void incrementAllNodeTypes() throws SecurityException, NoSuchMethodException {
		...
		int return_foo_iii = (int) e.getValue(ctx,Integer.TYPE);
		assertEquals(99,return_foo_iii);
		assertEquals(100,helper.foo.iii);

		e = parser.parseExpression("--foo.iii");
		assertEquals(100,helper.foo.iii);
		return_foo_iii = (int) e.getValue(ctx,Integer.TYPE);
		assertEquals(99,return_foo_iii);
		assertEquals(99,helper.foo.iii);
		...
		int r = (int) e.getValue(ctx,Integer.TYPE);
		assertEquals(3,r);
		assertEquals(4,((Integer)ctx.lookupVariable("wobble")).intValue());

		e = parser.parseExpression("--#wobble");
		assertEquals(4,((Integer)ctx.lookupVariable("wobble")).intValue());
		r = (int) e.getValue(ctx,Integer.TYPE);
		assertEquals(3,r);
		assertEquals(3,((Integer)ctx.lookupVariable("wobble")).intValue());
