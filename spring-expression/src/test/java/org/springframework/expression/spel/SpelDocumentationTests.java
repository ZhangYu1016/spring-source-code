...
public class SpelDocumentationTests extends AbstractExpressionTests {
	...
	public void testEqualityCheck() throws Exception {
		...
		boolean isEqual = (boolean) exp.getValue(context, Boolean.class);  // evaluates to true
		assertTrue(isEqual);
	}
...
//		teslaContext.setRootObject(tesla);
	...
	public void testMethodInvocation2() throws Exception {
		...
		societyContext.setRootObject(new IEEE());
		// evaluates to true
		boolean isMember = (boolean) parser.parseExpression("isMember('Mihajlo Pupin')").getValue(societyContext, Boolean.class);
		assertTrue(isMember);
	}
	...
	public void testRelationalOperators() throws Exception {
		boolean result = (boolean) parser.parseExpression("2 == 2").getValue(Boolean.class);
		assertTrue(result);
		// evaluates to false
		result = (boolean) parser.parseExpression("2 < -5.0").getValue(Boolean.class);
		assertFalse(result);

		// evaluates to true
		result = (boolean) parser.parseExpression("'black' < 'block'").getValue(Boolean.class);
		assertTrue(result);
	}
	...
	public void testOtherOperators() throws Exception {
		// evaluates to false
		boolean falseValue = (boolean) parser.parseExpression("'xyz' instanceof T(int)").getValue(Boolean.class);
		assertFalse(falseValue);

		// evaluates to true
		boolean trueValue = (boolean) parser.parseExpression("'5.00' matches '^-?\\d+(\\.\\d{2})?$'").getValue(Boolean.class);
		assertTrue(trueValue);

		//evaluates to false
		falseValue = (boolean) parser.parseExpression("'5.0067' matches '^-?\\d+(\\.\\d{2})?$'").getValue(Boolean.class);
		assertFalse(falseValue);
	}
	...
	public void testLogicalOperators() throws Exception {
		...
		// evaluates to false
		boolean falseValue = (boolean) parser.parseExpression("true and false").getValue(Boolean.class);
		assertFalse(falseValue);
		// evaluates to true
		String expression =  "isMember('Nikola Tesla') and isMember('Mihajlo Pupin')";
		boolean trueValue = (boolean) parser.parseExpression(expression).getValue(societyContext, Boolean.class);

		// -- OR --

		// evaluates to true
		trueValue = (boolean) parser.parseExpression("true or false").getValue(Boolean.class);
		assertTrue(trueValue);

		// evaluates to true
		expression =  "isMember('Nikola Tesla') or isMember('Albert Einstien')";
		trueValue = (boolean) parser.parseExpression(expression).getValue(societyContext, Boolean.class);
		assertTrue(trueValue);

		// -- NOT --

		// evaluates to false
		falseValue = (boolean) parser.parseExpression("!true").getValue(Boolean.class);
		assertFalse(falseValue);


		// -- AND and NOT --
		expression =  "isMember('Nikola Tesla') and !isMember('Mihajlo Pupin')";
		falseValue = (boolean) parser.parseExpression(expression).getValue(societyContext, Boolean.class);
		assertFalse(falseValue);
	}
	...
	public void testNumericalOperators() throws Exception {
		// Addition
		int two = (int) parser.parseExpression("1 + 1").getValue(Integer.class); // 2
		assertEquals(2,two);
		...
		// Subtraction
		int four =  (int) parser.parseExpression("1 - -3").getValue(Integer.class); // 4
		assertEquals(4,four);

		double d = (double) parser.parseExpression("1000.00 - 1e4").getValue(Double.class); // -9000
		assertEquals(-9000.0d, d, 0);

		// Multiplication
		int six =  (int) parser.parseExpression("-2 * -3").getValue(Integer.class); // 6
		assertEquals(6,six);

		double twentyFour = (double) parser.parseExpression("2.0 * 3e0 * 4").getValue(Double.class); // 24.0
		assertEquals(24.0d, twentyFour, 0);

		// Division
		int minusTwo =  (int) parser.parseExpression("6 / -3").getValue(Integer.class); // -2
		assertEquals(-2,minusTwo);

		double one = (double) parser.parseExpression("8.0 / 4e0 / 2").getValue(Double.class); // 1.0
		assertEquals(1.0d, one, 0);

		// Modulus
		int three =  (int) parser.parseExpression("7 % 4").getValue(Integer.class); // 3
		assertEquals(3,three);

		int oneInt = (int) parser.parseExpression("8 / 5 % 2").getValue(Integer.class); // 1
		assertEquals(1,oneInt);

		// Operator precedence
		int minusTwentyOne = (int) parser.parseExpression("1+2-3*8").getValue(Integer.class); // -21
	...
	public void testTypes() throws Exception {
		...
		boolean trueValue = (boolean) parser.parseExpression("T(java.math.RoundingMode).CEILING < T(java.math.RoundingMode).FLOOR").getValue(Boolean.class);
		assertTrue(trueValue);
	}
